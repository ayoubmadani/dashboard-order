        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className={sectionClass}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className={labelClass}>{t('form.name_label')} <span className="text-rose-500">{t('form.required')}</span></label>
                <input type="text" name="name" value={formData.name} onChange={handleInputChange} placeholder={t('form.name_placeholder')} className={inputClass(errors.name)} />
              ...
              <div>
                ...
                <input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="0557123456" className={inputClass(errors.phone)} dir="ltr" />
                {errors.phone && <p className="text-rose-500 text-xs mt-1">{errors.phone}</p>}
              </div>

              <div>
                <label className={labelClass}><Mail size={14} className="inline me-1" />{t('form.email_label')}</label>
                <input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="example@email.com" className={inputClass(errors.email)} dir="ltr" />
            ...
            <div className="space-y-6">
              <div>
                ...
                {heroImagePreview ? (
                  ...
                  <button type="button" onClick={openHeroModal} className="w-full h-48 border-2 border-dashed border-gray-300 dark:border-zinc-700 rounded-xl hover:border-indigo-500 transition-colors flex flex-col items-center justify-center gap-2 hover:bg-gray-50 dark:hover:bg-zinc-800">
          ...
          <div className={sectionClass}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">{t('form.top_bar')}</h2>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="showTopBar" checked={formData.showTopBar} onChange={handleInputChange} className="w-5 h-5 rounded text-indigo-600 focus:ring-indigo-500" />
                <span className="text-sm font-medium text-gray-700 dark:text-zinc-300">{t('form.top_bar_enable')}</span>
              </label>
            </div>
            {formData.showTopBar && (
              <div>
          ...
          <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/20 dark:to-purple-950/20 rounded-2xl border border-indigo-200 dark:border-indigo-800 p-6">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">{t('form.preview')}</h3>
              ...
              {[
                ...
                <div key={label} className="bg-white dark:bg-zinc-900 p-4 rounded-xl">
                  ...
                  <p className={`font-bold truncate flex items-center gap-1 ${highlight ? 'text-indigo-600' : 'text-gray-900 dark:text-white'}`}>
                    {val}{highlight && formData.domain && <ExternalLink size={12} />}
                  </p>
                </div>
              ))}
