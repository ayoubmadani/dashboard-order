...
function Section({ icon: Icon, iconColor, title, children, className = '' }) {
  return (
    <section className={`bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 shadow-sm ${className}`}>
      <div className="flex items-center gap-2.5 px-6 py-5 border-b border-gray-50 dark:border-zinc-800">
...
function ThemeCard({ image, name, isActivating, onActivate, isActive }) {
  ...
  return (
    <div className={`relative flex-shrink-0 w-52 rounded-2xl overflow-hidden border-2 transition-all duration-200 ${
      isActive
...
function ThemeCardPlan({ image, name, isActivating, onActivate, isActive }) {
  ...
  return (
    <div className={`relative flex-shrink-0 w-52 rounded-2xl overflow-hidden border-2 transition-all duration-200 ${
      ...
      <div className="h-32 bg-gray-100 dark:bg-zinc-800 overflow-hidden relative">
        <img src={image || DEFAULT_IMAGE} alt={name} className="w-full h-full object-cover" />
...
//  Gallery Card
// ─────────────────────────────────────────────
function GalleryCard({ item, isInstalled, isIncludedInPlan, installingId, onInstall, storeURL }) {
    const { t, i18n } = useTranslation('translation', { keyPrefix: 'theme' });
...
export default function Theme() {
  ...
  return (
    <div dir={isRtl ? 'rtl' : 'ltr'} className="space-y-6 font-sans animate-in fade-in duration-500">
      ...
      {/* ── My Themes ── */}
      <Section icon={Palette} iconColor="bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400" title={t('my_themes.title')}>
      ...
      {/* ── Plan Themes ── */}
      {planTheme.length > 0 && (
        <section className="bg-white dark:bg-zinc-900 rounded-2xl border-2 border-blue-100 dark:border-blue-900/30 shadow-sm relative overflow-hidden">
          <div className="absolute inset-0 bg-blue-500/[0.02] pointer-events-none" />
          <div className="flex items-center gap-2.5 px-6 py-5 border-b border-blue-50 dark:border-blue-900/20 relative z-10">
            <div className="p-2 bg-blue-100 dark:bg-blue-900/30 rounded-xl text-blue-600 dark:text-blue-400">
              <Crown size={17} />
            </div>
            <h2 className="text-base font-black text-gray-900 dark:text-white">{t('subscription.title')}</h2>
          </div>
      ...
      {/* ── Type filter ── */}
      <Section icon={Layers} iconColor="bg-violet-50 dark:bg-violet-900/20 text-violet-600 dark:text-violet-400" title={t('types.title')}>
      ...
      {/* ── Gallery ── */}
      <Section icon={LayoutGrid} iconColor="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400" title={t('gallery.title')}>
        {loading ? (
