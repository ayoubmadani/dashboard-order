...
// ─────────────────────────────────────────────
const STATUS_META = {
  pending:   { label: 'معلق',      color: '#f59e0b', bg: 'bg-amber-50   dark:bg-amber-900/20',   text: 'text-amber-600  dark:text-amber-400',   icon: Clock        },
  appl1:     { label: 'تأكيد 1',   color: '#6366f1', bg: 'bg-indigo-50  dark:bg-indigo-900/20',  text: 'text-indigo-600 dark:text-indigo-400',  icon: AlertCircle  },
  appl2:     { label: 'تأكيد 2',   color: '#8b5cf6', bg: 'bg-violet-50  dark:bg-violet-900/20',  text: 'text-violet-600 dark:text-violet-400',  icon: AlertCircle  },
  appl3:     { label: 'تأكيد 3',   color: '#a855f7', bg: 'bg-purple-50  dark:bg-purple-900/20',  text: 'text-purple-600 dark:text-purple-400',  icon: AlertCircle  },
  confirmed: { label: 'مؤكد',      color: '#3b82f6', bg: 'bg-blue-50    dark:bg-blue-900/20',    text: 'text-blue-600   dark:text-blue-400',    icon: CheckCircle2 },
  shipping:  { label: 'شحن',       color: '#06b6d4', bg: 'bg-cyan-50    dark:bg-cyan-900/20',    text: 'text-cyan-600   dark:text-cyan-400',    icon: Truck        },
  delivered: { label: 'مسلّم',     color: '#10b981', bg: 'bg-emerald-50 dark:bg-emerald-900/20', text: 'text-emerald-600 dark:text-emerald-400', icon: CheckCircle2 },
  cancelled: { label: 'ملغى',      color: '#ef4444', bg: 'bg-red-50     dark:bg-red-900/20',     text: 'text-red-600    dark:text-red-400',     icon: XCircle      },
  returned:  { label: 'مرجع',      color: '#f97316', bg: 'bg-orange-50  dark:bg-orange-900/20',  text: 'text-orange-600 dark:text-orange-400',  icon: RotateCcw    },
  postponed: { label: 'مؤجل',      color: '#64748b', bg: 'bg-slate-50   dark:bg-slate-900/20',   text: 'text-slate-600  dark:text-slate-400',   icon: Clock        },
};
...
function StatCard({ icon: Icon, label, value, sub, color, trend, loading }) {
  return (
    <div className={`relative bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-5 shadow-sm overflow-hidden group hover:shadow-md transition-shadow`}>
      {/* Decorative blob */}
      ...
      {trend !== undefined && !loading && (
        <div className={`flex items-center gap-1 mt-3 text-xs font-semibold ${trend >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
          ...
          {Math.abs(trend)}% مقارنة بالأسبوع الماضي
        </div>
      )}
    </div>
  );
...
function StatusRow({ status, count, total }) {
  const meta = STATUS_META[status] || { label: status, color: '#94a3b8', bg: 'bg-gray-50', text: 'text-gray-500', icon: Package };
    ...
    <div className="flex items-center gap-3 py-2.5">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1">
          <span className="text-sm font-semibold text-gray-700 dark:text-zinc-300">{meta.label}</span>
    ...
    <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-xl px-4 py-3 shadow-xl text-sm">
      ...
      {payload.map((p, i) => (
        <p key={i} className="tabular-nums font-black" style={{ color: p.color }}>
          {p.name}: {p.value?.toLocaleString('ar-DZ')} {p.name === 'الإيراد' ? 'DA' : ''}
        </p>
      ))}
    </div>
  );
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-gray-50 dark:border-zinc-800/60 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-400 dark:text-zinc-600">{orders} طلب</p>
      </div>
...
export default function Analytics() {
  ...
  const fetchData = async (silent = false) => {
    ...
    try {
      ...
      console.error(err);
      setError('فشل في جلب البيانات. تأكد من الاتصال بالخادم.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  // Revenue and order counts for stat cards
  const stats = useMemo(() => {
    ...
    // ── خسائر المرتجعات: مجموع priceLoss لكل طلب بحالة returned ──
    const returnedOrders      = orders.filter(o => o.status === 'returned');
    const returnedCount       = returnedOrders.length;
    const returnedLoss        = returnedOrders.reduce((s, o) => s + Number(o.priceLoss || 0), 0);
    const returnedLossMonth   = returnedOrders
      .filter(o => isThisMonth(o.createdAt))
      .reduce((s, o) => s + Number(o.priceLoss || 0), 0);

    return { revenue, todayRevenue, monthRevenue, totalStatusCount,
             todayOrders: todayOrders.length, weekOrders: weekOrders.length,
             monthOrders: monthOrders.length,
             cancelledCount, deliveredCount, shippingCount,
             returnedCount, returnedLoss, returnedLossMonth };
  }, [orders, statusCounts]);

  // Area chart data – daily orders over selected range
  ...
  const topProducts = useMemo(() => {
    ...
    orders.forEach(o => {
      ...
      if (!map[id]) map[id] = {
        id, name: o.product?.name || 'منتج غير معروف',
  ...
  // Top wilayas by returned orders
  const topReturnedWilayas = useMemo(() => {
    const map = {};
    orders
      .filter(o => o.status === 'returned')
      .forEach(o => {
        const name = o.customerWilaya?.ar_name || o.customerWilaya?.name || `ولاية ${o.customerWilayaId}`;
  ...
  // Returned customers phone numbers (unique, most recent first)
  const returnedPhones = useMemo(() => {
    return orders
      ...
      .map(o => ({
        ...
        name:     o.customerName,
        wilaya:   o.customerWilaya?.ar_name || o.customerWilaya?.name || `ولاية ${o.customerWilayaId}`,
        loss:     Number(o.priceLoss || 0),
        date:     new Date(o.createdAt).toLocaleDateString('ar-DZ'),
      }));
  }, [orders]);

  // Top wilayas (all orders)
  ...
  // Pie data for delivery type
  const deliveryTypeData = useMemo(() => {
    const home   = orders.filter(o => o.typeShip === 'home').length;
    const office = orders.filter(o => o.typeShip === 'office').length;
    return [
      { name: 'للمنزل', value: home,   color: '#6366f1' },
      { name: 'للمكتب', value: office, color: '#10b981' },
    ];
  }, [orders]);

  // ── Render ─────────────────────────────────
  return (
    <div dir={isRtl ? 'rtl' : 'ltr'} className="min-h-screen bg-gray-50 dark:bg-zinc-950 p-4 md:p-6 lg:p-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-gray-900 dark:text-white">
              {t('analytics.title', 'لوحة الإحصائيات')}
            </h1>
            <p className="text-xs text-gray-400 dark:text-zinc-500 mt-0.5">
              {t('analytics.subtitle', 'تتبع أداء متجرك وتحليل الطلبات')}
            </p>
          </div>
        </div>
        <button
          ...
          disabled={refreshing}
          className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-xl text-sm font-semibold text-gray-600 dark:text-zinc-400 hover:bg-gray-50 dark:hover:bg-zinc-800 shadow-sm transition-all"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard
          ...
          sub={`${fmt(stats.monthRevenue)} DA هذا الشهر`}
          color="#10b981"
          loading={loading}
        />
        <StatCard
        ...
        <StatCard
          ...
          sub={`${fmt(stats.todayRevenue)} DA اليوم`}
          color="#10b981"
          loading={loading}
        />
      </div>

      {/* ── Returned Losses Card ── */}
      <div className="mb-6">
        <StatCard
          icon={RotateCcw}
          label="خسائر المرتجعات"
          value={`${fmt(stats.returnedLoss)} DA`}
          sub={`${stats.returnedCount} طلب مرجع · ${fmt(stats.returnedLossMonth)} DA هذا الشهر`}
          color="#f97316"
          loading={loading}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        ...
        {/* Area chart – orders & revenue trend */}
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="orders"  name="الطلبات" stroke="#6366f1" strokeWidth={2} fill="url(#ordersGrad)"  dot={false} activeDot={{ r: 4 }} />
                <Area type="monotone" dataKey="revenue" name="الإيراد" stroke="#10b981" strokeWidth={2} fill="url(#revenueGrad)" dot={false} activeDot={{ r: 4 }} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-5 shadow-sm">
          <h2 className="text-sm font-bold text-gray-800 dark:text-zinc-200 mb-4">
            نوع التوصيل
          </h2>
          {loading ? (
            <div className="h-56 bg-gray-50 dark:bg-zinc-800/50 rounded-xl animate-pulse" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        {/* Status breakdown */}
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-5 shadow-sm">
          <h2 className="text-sm font-bold text-gray-800 dark:text-zinc-200 mb-1">
            توزيع حالات الطلبات
          </h2>
          <p className="text-xs text-gray-400 dark:text-zinc-600 mb-4">
            إجمالي {fmt(totalStatusCount)} طلب
          </p>
          {loading ? (
            <div className="space-y-3">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="h-10 bg-gray-50 dark:bg-zinc-800/50 rounded-xl animate-pulse" />
              ))}
            </div>
          ) : statusCounts.length === 0 ? (
            <p className="text-sm text-gray-400 dark:text-zinc-600 text-center py-10">لا توجد بيانات</p>
          ) : (
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-5 shadow-sm">
          ...
          {loading ? (
            <div className="space-y-3">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="h-12 bg-gray-50 dark:bg-zinc-800/50 rounded-xl animate-pulse" />
              ))}
            </div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-5 shadow-sm">
          ...
          {loading ? (
            <div className="space-y-3">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="h-8 bg-gray-50 dark:bg-zinc-800/50 rounded-xl animate-pulse" />
              ))}
            </div>
          ) : topWilayas.length === 0 ? (
            <p className="text-sm text-gray-400 dark:text-zinc-600 text-center py-10">لا توجد بيانات</p>
            ...
            <div className="space-y-1">
              {topWilayas.map(w => (
                <WilayaRow key={w.name} name={w.name} count={w.count} total={wilayaTotal} />
              ))}
            </div>
          )}
            ...
            <div className="mt-4">
              <ResponsiveContainer width="100%" height={100}>
                <BarChart data={topWilayas} margin={{ top: 0, right: 0, left: -30, bottom: 0 }}>
                  ...
                  <Tooltip formatter={(v) => [`${v} طلب`, 'العدد']} />
                  <Bar dataKey="count" name="الطلبات" radius={[4, 4, 0, 0]}>
                    {topWilayas.map((_, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
        ...
        {/* Top returned wilayas */}
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            <RotateCcw className="w-4 h-4 text-orange-500" />
            <h2 className="text-sm font-bold text-gray-800 dark:text-zinc-200">الولايات الأكثر إرجاعاً</h2>
          </div>
          <p className="text-xs text-gray-400 dark:text-zinc-600 mb-4">
            إجمالي {returnedWilayaTotal} مرتجع
          </p>
          {loading ? (
            <div className="space-y-3">
              {[...Array(5)].map((_, i) => (
          ...
          ) : topReturnedWilayas.length === 0 ? (
            <p className="text-sm text-gray-400 dark:text-zinc-600 text-center py-10">لا توجد مرتجعات</p>
          ) : (
            <div className="space-y-1">
        ...
        {/* Returned customers phones */}
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            <RotateCcw className="w-4 h-4 text-orange-500" />
            <h2 className="text-sm font-bold text-gray-800 dark:text-zinc-200">أرقام هواتف المرتجعين</h2>
          </div>
          <p className="text-xs text-gray-400 dark:text-zinc-600 mb-4">
            {returnedPhones.length} طلب مرجع
          </p>
          {loading ? (
            <div className="space-y-3">
              {[...Array(5)].map((_, i) => (
              ...
              {returnedPhones.map((r, i) => (
                <div key={i} className="flex items-center gap-3 py-2.5 border-b border-gray-50 dark:border-zinc-800/60 last:border-0">
                  ...
                  <div className="text-left shrink-0">
                    ...
                    <p className="text-xs text-orange-500 font-semibold tabular-nums text-left">{fmt(r.loss)} DA</p>
                  </div>
                </div>
              ))}
            </div>
